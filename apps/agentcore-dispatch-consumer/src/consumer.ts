...
export interface AgentCoreDispatchAlarm {
	raise(input: {
		reason:
			| "disabled_delivery"
			| "invalid_envelope"
...
export interface AgentCoreSqsEvent {
	Records: Array<{ messageId: string; body: string }>;
}

export interface AgentCoreSqsBatchResponse {
	batchItemFailures: Array<{ itemIdentifier: string }>;
}

export function createAgentCoreDispatchConsumer(options: {
	...
	async function processRecord(record: {
		...
		try {
			dispatch = parseAgentCoreDispatchEnvelope(record.body);
		} catch {
			await options.alarm.raise({
	...
	return {
		async handle(event: AgentCoreSqsEvent): Promise<AgentCoreSqsBatchResponse> {
			const failures: AgentCoreSqsBatchResponse["batchItemFailures"] = [];
