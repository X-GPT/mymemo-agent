import { createProductionAgentCoreRuntimeFromEnv } from "./production";
import { startRuntimeServer } from "./server";

const production = await createProductionAgentCoreRuntimeFromEnv(Bun.env);
...
production.logger.info({
	message: "AgentCore Runtime started",
...
async function stop(signal: NodeJS.Signals): Promise<void> {
	...
	stopping = true;
	production.logger.info({
		message: "AgentCore Runtime stopping",
		signal,
	});
	...
	production.logger.info({ message: "AgentCore Runtime stopped" });
	process.exit(0);
}
