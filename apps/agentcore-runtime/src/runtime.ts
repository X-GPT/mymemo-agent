...
interface PendingAcquisition {
	dispatch: AgentCoreDispatchIdentity;
	promise: Promise<CommittedAgentCoreAcquisition>;
}
...
function requireSameDispatch(
	...
	if (!sameAgentCoreDispatch(occupied, incoming)) {
...
export function createAgentCoreRuntime(
	...
	return {
		...
		async invoke(input: {
			...
			const dispatch = parseAgentCoreDispatchEnvelope(input.rawEnvelope);
			...
			let acquired: CommittedAgentCoreAcquisition;
			try {
