 ...
 */
export type EnsureVm = (ref: ConversationRef) => Promise<void>;
...
/** The `runHookPayload` ceiling the platform enforces (`run-microvm` has no env field). */
const RUN_HOOK_PAYLOAD_MAX_BYTES = 4096;
...
export function createEnsureVm(deps: EnsureVmDeps): EnsureVm {
	const { store, controlPlane, logger } = deps;
	...
	function runHookPayload(ref: ConversationRef, gatewayToken: string) {
		...
		if (Buffer.byteLength(payload) > RUN_HOOK_PAYLOAD_MAX_BYTES) {
			throw new Error("runHookPayload exceeds the platform's 4 KB limit");
		}
		return payload;
	}
	...
	async function launch(ref: ConversationRef): Promise<void> {
		...
		try {
			...
			await store.releaseClaim(ref).catch((releaseError) => {
				logger.error({ ...ref, err: releaseError }, "VM claim release failed");
			});
	...
	async function nudge(
		ref: ConversationRef,
		row: ConversationVmRow & { microvmId: string; endpoint: string },
	): Promise<"nudged" | "gone"> {
		try {
	...
	async function ensure(ref: ConversationRef, rehydrated: boolean) {
		...
		if (deps.upgradeUrgent) {
			...
			if (current !== undefined && claim.imageVersion !== current) {
				if (!(await store.claimUpgrade(ref, { microvmId: claim.microvmId }))) {
