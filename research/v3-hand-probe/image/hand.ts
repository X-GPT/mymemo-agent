	...
	async fetch(req) {
		...
		try {
			...
			switch (path) {
				...
				case "/grep": { const args = ["grep", "-rn", "-E", ...(body.glob ? [`--include=${body.glob}`] : []), "--", String(body.pattern), "."]; const r = await bash(args.map((a) => `'${a.replace(/'/g, `'\\''`)}'`).join(" "), 30_000); return json({ matches: r.stdout, truncated: r.truncated, exitCode: r.exitCode }); }
