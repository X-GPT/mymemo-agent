...
const handServer = createSdkMcpServer({
	...
	version: "0.0.1",
	tools: [
		tool("bash", "Run a shell command in the workspace sandbox. Returns stdout, stderr and the exit code.", { command: z.string(), timeout: z.number().optional().describe("ms") },
			async ({ command, timeout }) => { const r = await hand("bash", { command, timeoutMs: timeout ?? 120_000 }); return text(`exit ${r.exitCode}${r.timedOut ? " (timed out)" : ""}${r.truncated ? " (output truncated)" : ""}\n--- stdout ---\n${r.stdout}\n--- stderr ---\n${r.stderr}`, r.exitCode !== 0); }, { alwaysLoad: true }),
		tool("read", "Read a file in the workspace (path relative to the workspace root).", { file_path: z.string(), offset: z.number().optional(), limit: z.number().optional() },
			async (a) => { const r = await hand("read", { path: a.file_path, offset: a.offset, limit: a.limit }); return text(r.text); }, { alwaysLoad: true }),
		tool("write", "Create or overwrite a file in the workspace.", { file_path: z.string(), content: z.string() },
			async (a) => { await hand("write", { path: a.file_path, content: a.content }); return text(`wrote ${a.file_path}`); }, { alwaysLoad: true }),
		tool("edit", "Replace an exact string in a file. old_string must match exactly once unless replace_all.", { file_path: z.string(), old_string: z.string(), new_string: z.string(), replace_all: z.boolean().optional() },
			async (a) => { try { const r = await hand("edit", { path: a.file_path, old: a.old_string, new: a.new_string, replaceAll: a.replace_all }); return text(`edited ${a.file_path} (${r.replaced} replacement)`); } catch (e) { return text(String((e as Error).message), true); } }, { alwaysLoad: true }),
		tool("glob", "List workspace files matching a glob pattern.", { pattern: z.string(), path: z.string().optional() },
			async (a) => { const r = await hand("glob", a); return text(r.files.join("\n") || "(no matches)"); }, { alwaysLoad: true }),
		tool("grep", "Search workspace files for a regex. Returns file:line:match lines.", { pattern: z.string(), path: z.string().optional(), glob: z.string().optional() },
			async (a) => { const r = await hand("grep", { pattern: a.pattern, glob: a.glob }); return text(r.matches || "(no matches)"); }, { alwaysLoad: true }),
	],
});
