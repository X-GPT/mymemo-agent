...
import { hand, handGet, handStream, calls } from "./hand-client";
...
await r("egress example.com (expect FAIL)", () => hand("bash", { command: "curl -sS -m 5 -o /dev/null -w '%{http_code}' https://example.com || echo CURL_FAILED_$?" }));
...
{ const t0 = performance.now(); const gaps: number[] = []; let last = t0; await handStream("for i in 1 2 3 4 5; do echo tick $i; sleep 1; done", (type, d) => { const now = performance.now(); gaps.push(Math.round(now - last)); last = now; console.log(`  stream ${type} +${Math.round(now - t0)}ms ${JSON.stringify(d).slice(0, 80)}`); }); console.log("stream gaps ms:", gaps.join(",")); }
