...
async function waitForHistoryRecovery(
	conversationId: string,
	runId: string,
	timeoutMs: number,
): Promise<Response> {
	const deadline = Date.now() + timeoutMs;
	while (Date.now() < deadline) {
		const response = await fetchRunEvents(conversationId, runId);
		if (response.status === 410) return response;
		if (response.status !== 503) {
			throw new Error(
				`Run ${runId} returned ${response.status} while waiting for history recovery`,
			);
		}
		await response.body?.cancel();
		await Bun.sleep(50);
	}
	throw new Error(`Run ${runId} did not enter history recovery`);
}
...
describe.skipIf(!RUN)("AgentCore integration (real Postgres and Redis)", () => {
	...
	it(
		...
		async () => {
			...
			const recovering = await waitForHistoryRecovery(
				turn.conversationId,
				turn.runId,
				TURN_TIMEOUT_MS,
			);
	...
	it(
		...
		async () => {
			...
			const turn = await admitIntegrationRun(prompt);
			await startAgentCoreRuntime({ INTEGRATION_RESUME_DELAY_MS: "1500" });
	...
	it(
		...
		async () => {
			...
			for (const cap of caps) {
				...
				const recovery = await fetchRunEvents(turn.conversationId, turn.runId);
