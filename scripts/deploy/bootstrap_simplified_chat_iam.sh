...
confirm "Use this identity to update the deploy role in account 637423444544?" || exit 1

stage "Review the targeted Terraform plan"
work=$(mktemp -d)
trap 'rm -rf "$work"' EXIT
terraform -chdir=infra/bootstrap-iam init
terraform -chdir=infra/bootstrap-iam validate
terraform -chdir=infra/bootstrap-iam plan -var-file=prod.tfvars \
  -target=aws_iam_role_policy_attachment.simplified_chat -out="$work/bootstrap.tfplan"
terraform -chdir=infra/bootstrap-iam show -json "$work/bootstrap.tfplan" > "$work/plan.json"
# The role is a dependency; stop if Terraform also wants to change its trust policy.
...
confirm "Post this non-secret verification output to issue #749?" || exit 1
gh issue comment 749 --repo X-GPT/mymemo-agent --body-file "$work/evidence.txt"
finish
