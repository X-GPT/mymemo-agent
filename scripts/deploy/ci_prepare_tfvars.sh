...
if [[ -n "$chat_api_image" || -n "$agent_maintenance_image" || -n "$agentcore_dispatch_publisher_image" ]]; then
  if [[ -z "$chat_api_image" || -z "$agent_maintenance_image" || -z "$agentcore_dispatch_publisher_image" ]]; then
    echo "Set CHAT_API_IMAGE, AGENT_MAINTENANCE_IMAGE, and AGENTCORE_DISPATCH_PUBLISHER_IMAGE together, or set none and provide IMAGE_TAG." >&2
    exit 1
  fi
else
