...
agent_maintenance_image="${AGENT_MAINTENANCE_IMAGE:-}"
agentcore_dispatch_publisher_image="${AGENTCORE_DISPATCH_PUBLISHER_IMAGE:-}"

if [[ -n "$chat_api_image" || -n "$agent_worker_image" || -n "$agent_maintenance_image" || -n "$agentcore_dispatch_publisher_image" ]]; then
  if [[ -z "$chat_api_image" || -z "$agent_worker_image" || -z "$agent_maintenance_image" || -z "$agentcore_dispatch_publisher_image" ]]; then
    echo "Set CHAT_API_IMAGE, AGENT_WORKER_IMAGE, AGENT_MAINTENANCE_IMAGE, and AGENTCORE_DISPATCH_PUBLISHER_IMAGE together, or set none and provide IMAGE_TAG." >&2
    exit 1
  fi
else
  ...
  agent_maintenance_repository_url="$(terraform -chdir=infra/ecr output -raw agent_maintenance_ecr_repository_url)"
  ...
  agent_maintenance_image="${agent_maintenance_repository_url}:${image_tag}"
...
echo "  agent-maintenance image: ${agent_maintenance_image}"
