...
| `apps/chat-api/src/features/ai-chat/` | Harness-hosted AI SDK chat route (`POST /api/chat`, local composition only): one Claude Code turn per message on a per-turn `HarnessAgent` that runs Claude Code with `Read`/`Write`/`Edit`/`Grep` and no other built-in (`activeTools` is `HARNESS_ACTIVE_TOOLS` in `tools/`) in a persistent Vercel Sandbox per Conversation, streamed as the UI message stream |
