...
| `apps/in-vm-server/src/agent-session.ts` | ONE long-lived streaming-input `query()` carrying the Agent session across Turns, adapted to per-Turn windows; pinned to the Conversation id as its session id (#670) so a broken session, or a fresh VM with the Checkpoint restored, resumes the transcript instead of starting over |
