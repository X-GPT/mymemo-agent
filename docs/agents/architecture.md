...
| `apps/in-vm-server/src/app.ts` | The HTTP surface: nudge + health and the platform MicroVM lifecycle hooks; `/run` is the configuration boundary (503 nudges until it delivers `runHookPayload`) |
...
| `apps/in-vm-server/src/drain-loop.ts` | The Conversation's drain loop (#664): boot sweep, then claim-and-serve in sequence order, parking on a wakeable sleep (nudge doorbell + interval self-heal); an `error` Outcome never wedges it |
