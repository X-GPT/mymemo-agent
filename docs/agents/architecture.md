...
| `apps/in-vm-server` | The trusted In-VM server for /v2 (spec #654): claims queued Turns from Postgres, runs the confined Claude Agent SDK `query()`, maintains the Turn's single assistant UIMessage row commit-before-publish, publishes the v2 Turn Live Stream lane, and serves nudge + health plus the platform lifecycle hooks. Baked into the MicroVM image as its entrypoint (#666): boots unconfigured, then the `/run` hook's `runHookPayload` delivers the Conversation identity, data-plane URLs, and gateway token, the Checkpoint is restored (#670), and the boot sweep runs before serving; `/suspend` is the graceful-drain gate that writes the Checkpoint through chat-api. Runs locally against Postgres + Redis with the same config as plain env vars. |
...
| `packages/document-tools` | Portable document-tool implementation shared by chat-api's Harness chat path and the In-VM server (#665): the `ListDocuments`/`SearchDocuments`/`LoadDocuments` handlers and caps, frozen-Scope parsing, the scoped read-only KB client, and the `document_access_events` audit writer. Deliberately separate from the Run path's `documents/` module (#610). |
...
| `apps/chat-api/src/features/conversation-vm/` | /v2 MicroVM orchestration (#669): the `conversation_vm` registry + transactional launch claim (and the Checkpoint pointer, #670), the Lambda MicroVM control-plane client, and Ensure-VM (claim → `RunMicrovm` with the minted gateway token in `runHookPayload`, nudge, lazy rehydrate, urgent upgrade) |
| `apps/chat-api/src/features/checkpoint/` | The /v2 Checkpoint door (#670): `PUT`/`GET /v2/checkpoint/:conversationId` under the gateway token, streaming a VM's Checkpoint to and from S3 (`conversations/<id>/`) and moving the pointer guarded on the VM id |
...
| `apps/in-vm-server/src/app.ts` | The HTTP surface: nudge (optionally carrying the one command, `{interrupt: messageId}`) + health and the platform MicroVM lifecycle hooks; `/run` is the configuration boundary (503 nudges until it delivers `runHookPayload`) |
| `apps/in-vm-server/src/turn-serving.ts` | One-Turn drain: claim → confined SDK `query()` → per-Step assistant-row upsert (commit-before-publish) → Outcome terminalization; listens on the loop's interrupt channel for the life of its claim and ends `interrupted` with the committed snapshot (#668) |
| `apps/in-vm-server/src/drain-loop.ts` | The Conversation's drain loop (#664): boot sweep, then claim-and-serve in sequence order, parking on a wakeable sleep (nudge doorbell + interval self-heal); an `error` Outcome never wedges it. Applies the ephemeral interrupt command (#668): queued-cancel, or the SDK `interrupt()` on the in-flight Turn via the loop's interrupt channel. `pause`/`resume` are the suspend hook's graceful-drain gate (#670) |
| `apps/in-vm-server/src/agent-session.ts` | ONE long-lived streaming-input `query()` carrying the Agent session across Turns, adapted to per-Turn windows; pinned to a per-Conversation session id (#670) so a broken session, or a fresh VM with the Checkpoint restored, resumes the transcript instead of starting over |
| `apps/in-vm-server/src/checkpoint.ts` | The Checkpoint (#670): `~/.claude` + the Workspace as one tar, saved through and restored from chat-api's `/v2/checkpoint` door under the gateway token |
...
| `apps/in-vm-server/src/doc-tools.ts` | The document tools as in-process MCP (#665): per-call frozen-Scope binding, the in-flight Turn ref for audit attribution, and local-FS docs-cache materialization into the Workspace |
