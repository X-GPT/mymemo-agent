...
| `apps/agentcore-dispatch-consumer` | Production-shaped AgentCore dispatch boundary: transactional outbox publication, strict content-free SQS envelopes, exact acquisition, manual replay, and partial-batch acknowledgement. Its boundary ends after the exact Run reaches `running`. |
