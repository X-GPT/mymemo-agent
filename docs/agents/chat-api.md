...
audit writer live in the shared `@mymemo/document-tools` package (#665);
`ai-chat/tools/` keeps only the Harness `ToolSet` adapter. That shared implementation remains deliberately separate
