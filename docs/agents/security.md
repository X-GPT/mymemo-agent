...
chat-api's production `ApiConfig` must not hold the E2B credential, and holds the KB one only under the MicroVM rule two paragraphs below. It admits Runs, serves history and artifact metadata, attaches clients to Live Streams, and signs read-only artifact URLs. The one exception is the local-only `HarnessConfig` for the AI SDK chat path, which additionally holds the Vercel token triple, the OpenRouter credential, and the read-only `KB_DATABASE_URL`, because chat-api executes the document tools itself; the KB is touched only through `@mymemo/document-tools`' scoped client, constructed solely in `harness-chat-agent.ts`, and chat-api holds no E2B credential.

Per ADR-0034, the production `ApiConfig` does read the OpenRouter credential (`OPENROUTER_API_KEY`) and the gateway token secret (`GATEWAY_TOKEN_SECRET`) for the `/v2/gateway` route — the MicroVM's single door to the model provider. The rule those secrets live under: present in chat-api env only, never delivered to a VM, image, or Checkpoint. The VM authenticates with a per-Conversation signed token minted by chat-api (`features/gateway/gateway-token.ts`), which carries only `{ conversationId, userId, exp }`; the gateway validates it, injects the real key, forwards unbuffered, and logs per-Conversation usage. The same token opens the `/v2/checkpoint` door (#670): chat-api is the checkpoint bucket's only principal — it streams a VM's Checkpoint to and from exactly `conversations/<the token's conversation>/`, so per-Conversation scoping is enforced in code and the VM execution role carries no policy at all (the VM has no network path to S3 anyway). A Checkpoint is accepted only from the VM the `conversation_vm` row names (`x-mymemo-microvm-id`, self-asserted: a retired VM's token stays valid for hours, and the fence holds because it cannot learn its successor's id), so a retired VM cannot fork a Conversation's state. With MicroVM orchestration (#669) `ApiConfig` also reads the read-only `KB_DATABASE_URL` — solely to place it in the `runHookPayload` for the VM's trusted process, alongside the agent DB and Redis URLs; chat-api opens no KB connection of its own, and the payload never carries the OpenRouter key. chat-api's MicroVM authority is the control plane (`RunMicrovm`, `GetMicrovm`, `GetMicrovmImage`, `CreateMicrovmAuthToken`, `TerminateMicrovm`, passing the two network connectors) plus the Checkpoint bucket's `conversations/*` prefix. `env.test.ts` pins that `ApiConfig` reads the gateway credentials and the KB URL only under `MICROVM_IMAGE_ARN`, and never the E2B one; `gateway-token.test.ts` pins that the minted token can smuggle no provider key.

Inside the /v2 Execution runtime, a process boundary is the trust boundary (ADR-0034): the trusted In-VM server (`apps/in-vm-server`) alone holds the data-plane credentials (agent DB, Redis, read-only KB), and the Claude Code CLI it spawns is the untrusted surface. The document tools (`ListDocuments`, `SearchDocuments`, `LoadDocuments`, shared via `@mymemo/document-tools`) are served to the SDK loop as in-process MCP: Scope-enforced KB SQL, docs-cache materialization into the Workspace, and per-call `document_access_events` rows all execute in the trusted process, keyed by the in-flight Turn's user-message id. The CLI receives a credential-free allowlisted environment — the model credential rides along because in production it is the per-Conversation gateway token — under the #645 confinement bundle: `settingSources: []`, `strictMcpConfig`, `permissionMode: "dontAsk"` with cwd-scoped `Read(./**)`/`Edit(./**)` allows, and `Bash`/`BashOutput`/`KillShell` in `disallowedTools` ([ADR-0034 amendment](../adr/0034-run-the-chat-loop-in-per-conversation-lambda-microvms.md#amendment-2026-09-01--no-shell-in-the-vm)). `apps/in-vm-server/src/query-options.test.ts` pins the bundle, the denials, and the env allowlist.
