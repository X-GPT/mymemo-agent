...
**There is no shell.** `Bash` (with `BashOutput` and `KillShell`) is in `disallowedTools`, not merely absent from the allowlist. Sandbox-mode Bash cannot start in the MicroVM — bubblewrap cannot mount `/proc` there (#692) — and an unsandboxed shell would inherit the VM's network: IMDS and with it the execution role's cross-Conversation checkpoint scope, unbounded model spend on the gateway token, and a DNS exfiltration path. The fail-closed sandbox settings (`failIfUnavailable`, `allowUnsandboxedCommands: false`, network deny-all) stay in the bundle so that re-enabling a shell fails closed rather than silently running unconfined. `apps/in-vm-server/src/query-options.test.ts` pins the bundle, the denials, and the env allowlist.
