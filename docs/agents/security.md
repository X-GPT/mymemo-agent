...
**There is no shell.** `Bash` (with `BashOutput` and `KillShell`) is in `disallowedTools`, not merely absent from the allowlist, and nothing that served the shell remains — no bubblewrap or socat in the image, no `sandbox` settings in the SDK options or the policy tier. The [ADR-0034 amendment](../adr/0034-run-the-chat-loop-in-per-conversation-lambda-microvms.md) carries the reasoning. `apps/in-vm-server/src/query-options.test.ts` pins the tool denials and the env allowlist.
