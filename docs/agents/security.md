...
**There is no shell** (ADR-0034 amendment). `Bash` (with `BashOutput` and `KillShell`) is in `disallowedTools`, not merely absent from the allowlist. Sandbox-mode Bash cannot start in the MicroVM — bubblewrap cannot mount `/proc` there — and an unsandboxed shell would inherit the VM's network: IMDS and with it the execution role's cross-Conversation checkpoint scope, unbounded model spend on the gateway token, and a DNS exfiltration path. The image ships no bubblewrap or socat as a result. The fail-closed sandbox settings (`failIfUnavailable`, `allowUnsandboxedCommands: false`, network deny-all) stay in the bundle as the guard that makes any future shell fail closed rather than silently run unconfined. `apps/in-vm-server/src/query-options.test.ts` pins the bundle, the denials, and the env allowlist.
