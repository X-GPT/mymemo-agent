...
verification; chat-api does not invoke it in production. It receives no
database, E2B, Searchable document, Redis, or artifact credentials. Its resolved
OpenRouter credential is passed only to the Claude subprocess through the SDK's
per-query environment and is not written into the Runtime process environment.
