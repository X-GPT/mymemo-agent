...
Inside the /v2 Execution runtime, a process boundary is the trust boundary (ADR-0034): the trusted In-VM server (`apps/in-vm-server`) alone holds the data-plane credentials (agent DB, Redis), and the Claude Code CLI it spawns is the untrusted surface. The CLI receives a credential-free allowlisted environment — the model credential rides along because in production it is the per-Conversation gateway token — under the #645 confinement bundle: `settingSources: []`, `strictMcpConfig`, `permissionMode: "dontAsk"` with cwd-scoped `Read(./**)`/`Edit(./**)` allows, and OS-sandboxed Bash with network deny-all that fails closed (`failIfUnavailable`, `allowUnsandboxedCommands: false`). `apps/in-vm-server/src/query-options.test.ts` pins the bundle and the env allowlist.
