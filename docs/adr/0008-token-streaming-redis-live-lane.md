...
- **Message-granularity, Postgres-only** (implemented first) — cheap, exact replay,
...
- **Projector-side typewriter** — rejected: fake streaming; the worker only
  ...
  real-time-ness.
- **Legacy `text_delta` compatibility or historical payload backfill** —
  rejected: this is a coordinated hard cutover with no active users to migrate.
