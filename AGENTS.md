...
- **agentcore-canary-control** (`apps/agentcore-canary-control/`) - operator-only Lambda control path for one configured synthetic AgentCore Canary scenario. It verifies the deployment-owned non-human KB fixture before atomically creating the Campaign, AgentCore-canary Conversation, deterministic Run, and pending dispatch audit, then makes one immediate attempt through the shared leased publisher; it exposes no public route
- **agentcore-canary-dispatch** (`apps/agentcore-canary-dispatch/`) - production-shaped AgentCore dispatch boundary. It owns the strict content-free SQS envelope and post-commit Acquisition receipt, fail-closed SSM-gated immediate/repair publisher, audited manual replay, partial-batch SQS consumer acknowledgement matrix, AgentCore invocation adapter, and Runtime-side exact-acquisition boundary. Its boundary stops after the exact Run reaches `running`; the separate Runtime owns execution
- **agentcore-canary-runtime** (`apps/agentcore-canary-runtime/`) - request-oriented, Linux ARM64 AgentCore Runtime for one acquired synthetic Run. It exposes `/ping` and `/invocations`, holds one detached execution per process, resolves worker secrets from Secrets Manager, and delegates the already-running Run exactly once to shared `RunServing` without starting Fargate Claim, drain, expiration, Reclamation, or cleanup loops
- **agent-worker** (`apps/agent-worker/`) - split-runtime Fargate worker (MYM-47). Owns the worker-only credentials chat-api must not hold (read-only KB, OpenRouter, E2B) and runs the Postgres-backed run queue loop + Claude Agent SDK in trusted Fargate. Today: validated config, structured logger, worker id, bounded-concurrency supervisor, graceful drain, `/health`, the trusted-worker model client config (`src/model-client.ts`: OpenRouter env + default model for the Claude Agent SDK, per ADR-0003), the scoped document query client (`src/documents/` — frozen-scope-guarded KB list/search/fetch over `KB_DATABASE_URL`, audited to `document_access_events`, bounded model-safe errors) exposed through three model-facing tools (`list-documents-tool.ts` returns exact scoped inventory counts and metadata; `search-documents-tool.ts` returns citable passages; `load-documents-tool.ts` materializes documents-as-files per ADR-0004 — scope-checked, byte-capped content is written into the conversation's reserved docs cache `.mymemo/docs/` in the sandbox and only `{documentId, title, path, truncated}` metadata is returned, so no document body reaches run events), and the Conversation control plane plus shared Run-serving seam (Milestone 3, ADR-0023): `src/run-loop.ts` owns Claim, snapshot ordering, start, release, and heartbeat cadence, while `src/run-serving.ts` owns the already-running Run's lease renewal, interruption observation, Live Stream, processor supervision, artifact publication, and terminalization. Their unit is the Conversation (ADR-0015) — each tick renews unserved Claim windows, prompts attached Run heartbeats, and Claims Conversations up to the supervisor's capacity, then drains each Claim's snapshot Runs one at a time in submission order via an injected `RunProcessor` (`src/index.ts` wires the real SDK processor over `createStartRunQuery`; the synthetic processor has been removed from production). A successful turn terminalizes `done` directly — there is no end-of-turn checkpoint (ADR-0007): workspace persistence is the paused E2B sandbox itself, which idle-pauses once the turn stops renewing it and reconnects next turn with files intact. The SDK run loop sits behind that same `RunProcessor` seam (`src/sdk/`, Task 7.2): `run-processor.ts` starts a Claude Agent SDK query per claimed run and consumes its stream under supervision (`agent-stream.ts`) — appending canonical model content as sequence-numbered `run_events` only while `running`, atomically committing each Assistant completion with its validated `ui_payload` events, publishing those payloads after commit as `mymemo.generative_ui`, ignoring content after `interrupt_requested`, letting SDK errors terminalize as `error`, and interrupting the query on interruption/ownership-loss/shutdown; `run-tools.ts` binds the file/Bash/document tools plus the display-only `PresentUI` catalog tool to the run, and `RunLoop.stop()` aborts in-flight runs so shutdown interrupts active queries and cancels active E2B commands. Conversation continuity (ADR-0005, Task 7.3) mirrors the SDK transcript to Postgres through the worker `SessionStore` adapter (`sdk/session-store.ts`, over the shared `agent_sessions` helpers) run under a deterministic per-conversation query cwd so any worker resumes the prior turn; each qualifying `done` or `interrupted` terminal transaction publishes the first or later resume pointer `conversation_runtime.agent_session_id` atomically with the Outcome. The bound SessionStore supplies the successfully mirrored main-session id, while `consumeAgentStream` treats any `mirror_error` as a fail-fast bounded stop that aborts Tool/E2B work, so a still-running Run ends `error` while an already-committed interruption wins as `interrupted`, and the resume pointer is never established or advanced by that turn — while conversation deletion drops the transcripts in the cleanup sweep. `start-run-query.ts` (Task 9.5) fills the `startRunQuery` seam: it reads the run's `run_started` event through the shared `loadRunStartedTx` (message → plain-string prompt, frozen scope → document scope, fail-closed parse), ensures the runtime row idempotently and provisions the workspace connect-or-create through the injected `SandboxProvisioner`, persists a fresh sandbox via the fenced pointer update (a fence rejection kills the escaped sandbox — orphan-recording it if the kill fails — and leaves the Run for Reclamation; every pointer replace orphan-records the prior sandbox), keeps the sandbox awake with a per-run monotonic renewal timer (`sandbox-renewal.ts`) whose failure aborts the linked controller so the turn ends `error`, never `done`, and builds the fail-closed ADR-0006 query options (built-ins disabled, no settings sources, `dontAsk` + the executor allowlist, static system prompt, model-client env spread over the worker process env plus an ephemeral `CLAUDE_CONFIG_DIR`, the boot-verified pinned CLI path). The entrypoint now resolves and exec-verifies the SDK's native CLI before claiming, supplies the validated sandbox/file/Bash limits, creates the per-conversation Fargate cwd before each query, and wires `createSdkRunProcessor` over the real `startRunQuery`; failed turns log their internal error worker-side but persist only a generic client message. Successful turns compare start/end manifests under `/home/user/artifacts`, ledger fresh private object keys before upload, and commit changed current-artifact metadata atomically with `run_done`
- **@mymemo/agent-db** (`packages/agent-db/`) - shared writable-DB data layer (`mymemo_agent`): the Drizzle schema, the `Database` client, the concurrency-critical Run-store transaction helpers (`startClaimedRunTx`/`appendRunEventTx`/`transitionRunTerminalTx`/`requestRunInterruptionTx`/`reclaimConversationTx`/`expireUnownedQueuedRunsTx`/`admitQueuedRunTx`/`loadExecutingRunTx`/`loadRunStartedTx`), leased AgentCore outbox publication/manual replay and atomic exact acquisition, the shared live Conversation Ownership predicates for Run state, runtime, and transcript mutations, the fenced `conversation_runtime`/`orphan_sandboxes` helpers, the SDK session-transcript mirror helpers over the worker-only `agent_sessions` table, the canonical `run_events.type` write-side vocabulary (`RunEventType`), the PGlite test harness, and the migrations. The Claim and Reclamation protocol's concurrency properties — `SKIP LOCKED` contention, the Claim-versus-admission snapshot boundary, concurrent reclaimers, and a superseded holder racing its successor — are the one thing a single in-process backend cannot express, so they live in `src/conversation-ownership.postgres.test.ts`, gated on `AGENT_DATABASE_URL` and run by the CI `integration` job against the Postgres major production runs. AgentCore publisher/acquisition races share that real-Postgres gate in `src/canary-control.postgres.test.ts`. Both runtimes import the package so queue protocol, runtime-persistence fence, and permanent-history event vocabulary share one definition over one `pg` driver. Runtime-store performs sandbox/taint and Agent-session pointer writes; Run-store composes its named operations — including pointer publication into qualifying terminal transactions through the live Conversation Ownership fence, plus atomic Conversation Reclamation that terminalizes started Active Runs, taints the Workspace when command cleanup is unproven, and clears Ownership — while chat-api's `PostgresRunStore` composes transaction-scoped shared admission with the Conversation lifecycle lock
...
4. `POST /v1/conversations/:conversationId/runs/:runId/interrupt` — canonical durable interruption (ADR-0013). A queued Run terminalizes to `interrupted`, while a running or already-`interrupt_requested` Run becomes/remains `interrupt_requested`; both return `202 { runId, status }`, and a retry after the interruption won stays `202 { status: "interrupted" }`. A `done`/`error` Run returns `409`; a missing or foreign Run returns the same ownership-safe `404`. Interruption bypasses the new-work exposure gate.
...
7. The Conversation endpoints live in `src/features/conversations/`; permanent history lives in `src/features/conversation-history/`; artifacts live in `src/features/artifacts/`. `admitAgUiRun` is the sole Run admission path and transactionally writes the queued `runs` row plus `run_started`, preserving client Run/message identity for exact retry under the Conversation lifecycle lock. Backpressure is admission's own explicit Active Run count, taken under that same Conversation row lock — the `runs_one_active_per_conversation` partial unique index that used to imply it is gone.
...
**Exposure gate (`src/features/exposure-gate/`):** new agent work is gated by a server-side Statsig gate (`mymemo_agent_split_runtime_enabled`), evaluated on the **trusted identity** (never the body) **after** identity parse and **before** any Conversation or Run write. Denied → `403 { error: "Agent is not enabled" }`. It fails closed. Reconnect, interruption, history, and artifact access for existing owned resources do not consult it.

The sandboxed agent is treated as untrusted (it runs prompt-injectable, Bash-capable code). In the split runtime, chat-api does not mint sandbox credentials or hold provider/document/E2B secrets. The trusted execution runtimes — the Fargate `agent-worker` and the AgentCore canary Runtime — own model traffic, scoped document access, and E2B execution; secrets must not be placed into E2B sandbox env.

chat-api holds no provider/document/E2B secrets. It admits Runs, attaches to AG-UI Live Stream relays, reads permanent history and Downloadable-artifact metadata, and signs read-only artifact URLs. The trusted Fargate worker and AgentCore canary Runtime own model traffic, scoped document access, E2B sandbox creation, relay event production, and Downloadable-artifact publication.

### Key Modules

| Path | Purpose |
|------|---------|
| `src/features/conversations/` | Conversation resources, strict AG-UI Run admission, relay-backed SSE/reconnect, and canonical interruption |
...
| `src/features/run-store/` | Strict Run admission, ownership-scoped Run reads, and durable interruption over the shared queue helpers |
...
| `src/features/conversation-runtime-store/` | Thin re-export of the shared `@mymemo/agent-db/runtime-store`: persistent E2B workspace metadata over `conversation_runtime` (sandbox pointer, taint state, agent session resume pointer) — every worker mutation fenced on the live Conversation Ownership epoch and lease — plus the deliberately unfenced `orphan_sandboxes` recovery ledger. The helpers live in the shared package so the worker writes through the same fence |
| `src/config/env.ts` | Environment validation |
| `apps/agent-worker/src/run-loop.ts` | Fargate-only `RunLoop` control plane: global queued expiration and Reclamation, Conversation Claim, bounded snapshot drain in submission order, and unconditional release. It renews the Claim only while no Run is attached (start/between-Run windows), starts each snapshot Run under the epoch fence, and delegates the already-running Run and its heartbeat to `run-serving.ts`. It halts without release on Ownership loss and releases after graceful shutdown so unstarted snapshot Runs requeue immediately. The optional `RunDoorbell` provides low-latency pickup while timer ticks remain authoritative |
| `apps/agent-worker/src/run-serving.ts` | Shared `serveStartedRun` behavior (ADR-0023) for an already-running Run under one live Conversation Ownership epoch. It owns lease renewal and durable interruption observation, Live Stream production/degradation, SDK and Tool processing through `RunProcessor`, durable model content, Agent-session evidence, artifact publication, terminalization, and shutdown/Ownership-loss reconciliation. It returns a typed terminal, Ownership-loss, or shutdown result and contains no Claim, ordering, expiration, Reclamation, or release behavior |
...
| `apps/agent-worker/src/sdk/` | SDK run supervision and production query wiring: `run-tools.ts` exposes the fail-closed executor allowlist including `PresentUI`; `agent-stream.ts` emits standard Assistant text, atomically appends each Assistant completion followed by validated `ui_payload` events through the sequence-returning model-content seam, publishes their `mymemo.generative_ui` CUSTOM events only after commit, and wholly excludes `PresentUI` from Tool projection. It also persists MyMemo-identified, correlated Tool lifecycle events before their live AG-UI projections while `running`, interrupts on abort, and fail-fast stops on `mirror_error` while reporting mirror reliability for supervisor reconciliation. The bound SessionStore supplies direct main-session mirror evidence, which `run-processor.ts` preserves as bounded turn metadata when the consumer reports a typed, otherwise-reliable stream failure so a concurrent durable interruption can reconcile continuity. The remaining tool/session/query/sandbox modules retain their documented ADR-0004/0005/0006/0012/0017 responsibilities. `src/index.ts` wires the SDK processor and Redis Live Stream relay into the live worker |
| `packages/live-text/` | Shared required Redis URL validation, producer-buffered in-memory/Redis relay implementations, event validation and bounds, and payload-safe delivery telemetry |
| `packages/agent-db/` | Shared writable-DB data layer imported by chat-api + agent-worker: `src/schema.ts`, `src/client.ts` (`createDatabase`, `DbTx`), `src/conversation-ownership.ts` (all of ADR-0015: the live Ownership fence and its shared fence-first lock helper, plus the Claim protocol — claim-plus-snapshot in one transaction, unconditional release, epoch-fenced lease renewal — which the worker's drain executes through), `src/run-store.ts` (queue helpers plus epoch-fenced Run start, append, terminal, Live Stream marker writes, read-only executing-Run status observation, and Conversation Reclamation with the shared typed `RunWriteRejected` vocabulary), `src/runtime-store.ts` (fenced sandbox/taint helpers, the named in-transaction `agent_session_id` update, the Reclamation taint operation, and `orphan_sandboxes`), `src/session-store.ts` (worker-fenced SDK append/delete helpers plus deliberately unfenced transcript reads and administrative Conversation deletion over `agent_sessions`, ADR-0005), `src/artifact-store.ts` (pre-upload `artifact_objects` ledger and atomic pointer/current-metadata/`run_done` commit behind a fence-first Run lock), `src/run-events.ts` (`RunEventType` vocabulary), `src/testing.ts` (PGlite harness + `seedQueuedRun`), `src/migrations.ts` (`MIGRATIONS_DIR`), `drizzle/` migrations |
...
- `PORT` (default: `8080`), `LOG_LEVEL` (default: `info`), and `WORKER_HEARTBEAT_INTERVAL_MS` (default: `15000`). Runtime shutdown grace is fixed at 30 seconds and concurrency is fixed at one execution
...
Required:
- `AGENT_DATABASE_URL` — writable `mymemo_agent` DB (Runs, Run events, Conversation runtime, and Ownership state). Same DB chat-api uses; the worker Claims and renews Conversations, then starts and terminalizes their Runs
...
Optional:
- `WORKER_MAX_CONCURRENT_CONVERSATIONS` (default: `2`) — conservative per-task Conversation concurrency. One supervisor slot is held for a whole Conversation drain, not per Run; concurrent drains share the task's CPU/memory
...
- `WORKER_HEARTBEAT_INTERVAL_MS` (default: `15000`) — how often active Conversation drains renew their Ownership lease and observe executing-Run interruption state
