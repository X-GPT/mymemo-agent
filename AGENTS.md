...
| `apps/agent-worker/src/sdk/` | Milestone 7 SDK run supervision: `agent-stream.ts` (`consumeAgentStream` — persists assistant text while `running`, interrupts on abort, throws on SDK error / interrupted-clean-end; reports the session id + `mirror_error` for continuity), `run-tools.ts` (`buildRunTools`/`createRunMcpServer` — the file/Bash/document executor tools bound to the run binding as an in-process SDK MCP server), `run-processor.ts` (`createSdkRunProcessor` + the `startRunQuery` seam), `session-store.ts` (Task 7.3: `createConversationSessionStore` — the Postgres `SessionStore` adapter — plus `conversationWorkingDirectory`/`buildAgentSessionQueryConfig`). Consumed once E2B provisioning lands |
