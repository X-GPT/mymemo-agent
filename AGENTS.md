...
8. The retained Redis Stream is the only live/reconnect transport. It contains standard cursor-bearing AG-UI `RUN_STARTED`, Assistant text lifecycle, Tool lifecycle, and terminal events. Permanent Assistant messages, Tool activity, and Outcomes commit to Postgres before their matching completion events are appended to Redis. Runtime Redis failure degrades live delivery without changing service health or Run execution.
