...
- **agentcore-canary-control** (`apps/agentcore-canary-control/`) - operator-only Lambda control path for one configured synthetic AgentCore Canary scenario. It verifies the deployment-owned non-human KB fixture before atomically creating the Campaign, AgentCore-canary Conversation, deterministic Run, and pending dispatch audit; it neither publishes the outbox nor exposes a public route
