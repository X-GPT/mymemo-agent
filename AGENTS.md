...
| `src/features/conversation-runtime-store/` | Pre-existing compatibility re-export over shared `@mymemo/agent-db/runtime-store` records and helpers. It currently has no source importers and is retained unchanged in purpose under the repository's surgical-change rule |
