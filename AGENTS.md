...
| `src/features/conversation-runtime-store/` | Legacy re-export surface over shared `@mymemo/agent-db/runtime-store` records and helpers: persistent E2B workspace metadata over `conversation_runtime` (sandbox pointer and taint state) — every mutation fenced on the claiming run's `locked_by`/`locked_until` — plus the unfenced `orphan_sandboxes` recovery ledger. Run-store composes the named in-transaction Agent-session pointer publication through the same shared fence |
