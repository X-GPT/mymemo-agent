...
5. `GET /v1/conversations/:conversationId/history` — owner-scoped permanent history, paged as complete Runs with standard AG-UI messages and a separate terminal event. Postgres remains authoritative after the temporary Redis Stream expires or fails.
...
8. The retained Redis Stream is the only live/reconnect transport. It contains standard AG-UI `RUN_STARTED`, Assistant text lifecycle, Tool lifecycle, and terminal events. Permanent Assistant messages, Tool activity, and Outcomes commit to Postgres before their matching completion events are appended to Redis. Runtime Redis failure degrades live delivery without changing service health or Run execution.
...
chat-api holds no provider/document/E2B secrets. It admits Runs, consumes retained AG-UI Streams, reads permanent history and Downloadable-artifact metadata, and signs read-only artifact URLs. The trusted `agent-worker` owns model traffic, scoped document access, E2B sandbox creation, Redis event production, and Downloadable-artifact publication.

### Key Modules

| Path | Purpose |
|------|---------|
| `src/features/conversations/` | Conversation resources, strict AG-UI Run admission, retained Stream SSE/reconnect, and canonical cancellation |
...
| `packages/live-text/` | Shared required Redis URL validation, retained per-Run Stream store, and payload-safe delivery telemetry |
...
- `REDIS_URL` — authenticated `rediss://` secret for the retained per-Run AG-UI Stream. Missing, malformed, unauthenticated, or non-TLS configuration fails startup. Never logged
...
- `REDIS_URL` — authenticated `rediss://` secret for the retained per-Run AG-UI Stream. Missing, malformed, unauthenticated, or non-TLS configuration fails startup. Never logged or passed into E2B
