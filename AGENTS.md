...
chat-api holds no provider/document/E2B secrets. It queues runs, projects durable run events, reads Downloadable-artifact metadata, and signs read-only artifact URLs. The trusted `agent-worker` owns model traffic, scoped document access, E2B sandbox creation, and future artifact publication.
