...
| `src/features/conversation-runtime-store/` | Compatibility re-export surface over shared `@mymemo/agent-db/runtime-store` helpers and the user-bound mutation-owner type from `run-ownership`: persistent E2B workspace metadata over `conversation_runtime` (sandbox pointer, taint state, and the named in-transaction Agent-session pointer operation) — every mutation fenced on the claiming run's `locked_by`/`locked_until` — plus the unfenced `orphan_sandboxes` recovery ledger. Run-store composes pointer publication into the terminal transaction through the same shared fence |
