...
- **chat-api** (`apps/chat-api/`) - AI chat service; owns conversation creation, queued run insertion, durable SSE projection over Postgres `run_events`, best-effort merging of cursorless Live preview on the original request or an authorized active-Run reconnect, and ownership-checked listing/signing of the conversation's current Downloadable artifacts
...
chat-api holds no provider/document/E2B secrets. It queues runs, projects durable run events, reads Downloadable-artifact metadata, and signs read-only artifact URLs. The trusted `agent-worker` owns model traffic, scoped document access, E2B sandbox creation, and Downloadable-artifact publication.

### Key Modules

| Path | Purpose |
|------|---------|
| `src/features/conversations/` | `conversations.route.ts` (conversation creation/events), `conversations.controller.ts` (`createConversation` freezes scope; `queueConversationTurn` creates queued runs) |
...
| `apps/agent-worker/src/run-loop.ts` | `RunLoop` — the worker control loop over the shared helpers: `tick()` heartbeats owned runs (renew/observe cancel) then claims+dispatches queued runs to capacity; `start()`/`stop()` schedule it and drain (`stop()` aborts in-flight runs so shutdown interrupts them). A processor may return a `TurnResult` naming the agent session to advance. `finish()` terminalizes success as `done` directly (ADR-0007); failures are logged with their internal detail but persist only the generic client message `Run failed`. An optional injected `RunDoorbell` (`run-doorbell.ts`: `PostgresRunDoorbell` LISTENs on `run_queued`, notified by the `runs_notify_queued` trigger on every queued-run insert) rings an immediate coalesced tick so pickup latency is milliseconds; delivery is best-effort and the timer tick remains the source of truth |
...
| `apps/agent-worker/src/sdk/` | SDK run supervision and production query wiring: `agent-stream.ts` persists assistant text and client-safe tool events while `running` (text first, then `tool_use` in block order at each envelope commit; `tool_result` from non-replay user messages via a per-run id map), interrupts on abort, and reports the session id + `mirror_error`; `tool-event-projection.ts` owns the ADR-0009 executor-name allowlist and the bounded per-tool projections for all nine executor tools under the 16 KiB never-split event cap; `run-tools.ts` binds the file/Bash/document tools to the run; `run-processor.ts` provides `createSdkRunProcessor`; `session-store.ts` provides Postgres continuity and the deterministic query cwd; `start-run-query.ts` provides fenced connect-or-create provisioning and fail-closed ADR-0006 query options; `sandbox-renewal.ts` keeps the E2B workspace awake during a turn; `claude-code-executable.ts` resolves the SDK-owned non-musl platform binary and exec-verifies it at boot. `src/index.ts` wires these into the live worker |
