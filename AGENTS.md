...
7. On the new `/runs` path, the retained Redis Stream is the authoritative live/reconnect transport and contains standard `RUN_STARTED`, Assistant text lifecycle, and terminal AG-UI events. Permanent history and Outcome remain authoritative in Postgres and commit before their corresponding Redis completion events. Until #344 removes it, the legacy `/events` path continues to project recorded Postgres events and merge best-effort cursorless Live preview. Its frames are:
   ...
   - `tool_use` — `{ tool, arguments, truncated }`, one Tool invocation (ADR-0009): the short public tool name and a bounded, client-safe projection of its arguments; durable and replayable, carries its Run-event SSE cursor
   - `tool_result` — `{ tool, result, isError, truncated }`, one Tool result as a separate chronological item (no correlation id); an `isError` result carries the fixed safe message; a canceled/failed run may end after `tool_use` with no result
...
| Path | Purpose |
|------|---------|
| `src/features/conversations/` | Strict request validation and SSE; the controller freezes Conversation scope, orchestrates idempotent AG-UI admission, retains legacy queuing, and scopes interruption |
...
| `apps/agent-worker/src/sdk/` | SDK run supervision and production query wiring: `agent-stream.ts` emits standard Assistant text start/content, persists the canonical complete Assistant message, then emits text end; it also persists client-safe tool events while `running`, interrupts on abort, and reports session id + `mirror_error`. The remaining tool/session/query/sandbox modules retain their documented ADR-0004/0005/0006/0009 responsibilities. `src/index.ts` wires the SDK processor and Redis Stream store into the live worker |
