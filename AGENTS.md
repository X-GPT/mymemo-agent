...
| `apps/agent-worker/src/run-loop.ts` | `RunLoop` — the worker control loop over the shared helpers: `tick()` heartbeats owned runs (renew/observe cancel) then claims+dispatches queued runs to capacity; `start()`/`stop()` schedule it and drain. A processor may return a `TurnResult` (dirty state + the sandbox to checkpoint); `finish()` routes a successful turn through the snapshot barrier before `done`. `src/synthetic-processor.ts` is the Milestone 3 processor (returns nothing → a clean, sandbox-less turn) |
