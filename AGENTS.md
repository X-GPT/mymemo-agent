...
| `apps/agent-worker/src/run-loop.ts` | `RunLoop` — the worker control loop over the shared helpers, with the **Conversation** as its unit (ADR-0015): Claim/renew/drain plus best-effort relay-backed AG-UI production through `RunLiveStream`. One supervisor slot is held for a whole drain, which serves the Claim's snapshot Runs one at a time in submission order (mid-drain arrivals are left for a later Claim), starting each under the epoch fence and terminalizing it before the next. A lost Ownership lease halts the drain, abandons its Runs without terminalizing them, and deliberately does **not** release; a `status` rejection skips that Run; a deleted Conversation stops the drain with nothing to release; shutdown terminalizes the Run in flight, then releases so unstarted snapshot Runs requeue immediately. Each renewal also reads the executing Run's status to observe durable interruption; that read grants no authority, because every write is fenced solely by the live Conversation Ownership epoch. At-most-one-executing is program order in this single drain loop rather than a database invariant. It opens one producer per served Run, exposes `RUN_STARTED` after start, publishes terminal AG-UI only after the matching Postgres transition, then closes the producer and frees its buffer. Relay failure disables only live output. `finish()` still terminalizes success directly per ADR-0007. The optional `RunDoorbell` provides low-latency pickup while timer ticks remain authoritative |
