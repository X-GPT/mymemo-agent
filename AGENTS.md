...
| `apps/agent-worker/src/run-loop.ts` | `RunLoop` — the worker control loop over the shared helpers: `tick()` heartbeats owned runs (renew/observe cancel) then claims+dispatches queued runs to capacity; `start()`/`stop()` schedule it and drain (`stop()` aborts in-flight runs so shutdown interrupts them). A processor may return a `TurnResult` (the agent session to advance the resume pointer to); `finish()` terminalizes a successful turn `done` directly — no end-of-turn checkpoint (ADR-0007). `src/synthetic-processor.ts` is the Milestone 3 processor (returns nothing → no session to record) |
| `apps/agent-worker/src/sdk/` | Milestone 7 SDK run supervision: `agent-stream.ts` (`consumeAgentStream` — persists assistant text while `running`, interrupts on abort, throws on SDK error / interrupted-clean-end; reports the session id + `mirror_error` for continuity), `run-tools.ts` (`buildRunTools`/`createRunMcpServer` — the file/Bash/document executor tools bound to the run binding as an in-process SDK MCP server), `run-processor.ts` (`createSdkRunProcessor` + the `startRunQuery` seam), `session-store.ts` (Task 7.3: `createConversationSessionStore` — the Postgres `SessionStore` adapter — plus `conversationWorkingDirectory`/`buildAgentSessionQueryConfig`), `start-run-query.ts` (Task 9.5: `createStartRunQuery` — fenced connect-or-create provisioning, orphan bookkeeping, the static MyMemo system prompt, and the fail-closed ADR-0006 query options over injected provisioner/janitor/SDK-query seams), `sandbox-renewal.ts` (the per-run monotonic keep-alive timer). Wired into the entrypoint in Task 9.6 |
