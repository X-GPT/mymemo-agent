...
- **chat-api** (`apps/chat-api/`) - AI chat service; owns conversation creation and lifecycle management, strict/idempotent AG-UI Run admission, retained per-Run Redis Stream consumption for original POST and reconnect, the legacy durable SSE projection over Postgres `run_events` plus cursorless Live preview until #344 removes it, and ownership-checked listing/signing of the conversation's current Downloadable artifacts
...
The chat and Downloadable-artifact surface is mounted under `/v1` in `src/routes/v1.ts`. A `conversation` is the durable container, `events` are what you append to it, and artifacts are the current named files available for download:
...
3. `POST /v1/conversations/:conversationId/runs` — strictly validate one standard `RunAgentInput`, require `threadId` to equal the owned Conversation id, reject client Tools/state/forwarded authority, and atomically admit the client-supplied `runId` plus final plain-text User message through `admitAgUiRun`. Exact retries reattach to the same logical Run; mismatched reuse returns `409`. Admission commits before any Redis access. The response and `GET /v1/conversations/:conversationId/runs/:runId/events` consume the same retained per-Run Redis Stream, emitting standard AG-UI JSON in SSE `data` with the Redis cursor in `id`. A Redis failure before the first event returns retryable `503`; a later failure closes the incomplete stream without a synthesized protocol event.
4. `POST /v1/conversations/:conversationId/runs/:runId/cancel` — canonical durable cancellation. The route validates trusted identity and path-safe ids, then calls the ownership-scoped `runStore.requestCancellation` directly; it never creates or translates a `user.interrupt` event. A queued Run terminalizes to `canceled`, while a running or already-`cancel_requested` Run becomes/remains `cancel_requested`; both return `202 { runId, status }`. A terminal Run returns `409 { runId, status }`; a missing or foreign Run returns the same ownership-safe `404`. Cancellation controls existing work, so it bypasses the new-work exposure gate and remains available while clients are reconnecting or recovering.
5. `POST /v1/conversations/:conversationId/events` — legacy append-and-stream surface retained until #344. With:
...
6. `GET /v1/conversations/:conversationId/artifacts` and `GET /v1/conversations/:conversationId/artifacts/:artifactId/download-url` — list the current Downloadable set or return `{ downloadUrl }` with a fresh five-minute S3 URL. Both routes parse the same trusted identity, verify conversation ownership before artifact lookup, return `404` for missing/foreign resources, and bypass the new-work exposure gate. Downloads are forced attachments with the stored server-selected content type; chat-api never proxies object bytes.
7. The conversation endpoints live in `src/features/conversations/` — `conversations.route.ts` owns validation, SSE, and the canonical cancellation route's direct ownership-scoped store call; `conversations.controller.ts` owns Conversation creation, legacy queuing, strict AG-UI admission orchestration, and the temporary legacy `user.interrupt` path. No upstream API calls here.
8. `queueConversationTurn` remains the sole legacy `user.message` path; `admitAgUiRun` is the new strict AG-UI path. Both create a queued `runs` row and `run_started` event transactionally through `PostgresRunStore`; AG-UI admission additionally preserves the client Run/message identity for exact retry and shares the transaction with the Conversation lifecycle lock. The `runs_one_active_per_conversation` partial unique index enforces backpressure. Scope always comes from the frozen Conversation record. Actual model execution, E2B sandbox use, document access, and terminal/content appends are worker responsibilities.
9. On the new `/runs` path, the retained Redis Stream is the authoritative live/reconnect transport and contains standard `RUN_STARTED`, Assistant text lifecycle, and terminal AG-UI events. Permanent history and Outcome remain authoritative in Postgres and commit before their corresponding Redis completion events. Until #344 removes it, the legacy `/events` path continues to project recorded Postgres events and merge best-effort cursorless Live preview. Its frames are:
   ...
   - `TOOL_CALL_START` / `TOOL_CALL_ARGS` / `TOOL_CALL_END` — one correlated Tool invocation (ADR-0012): a MyMemo-generated `toolCallId`, its owning Assistant `parentMessageId`, the short public Tool name, and bounded client-safe arguments; each durable lifecycle frame carries its Run-event SSE cursor
   - `TOOL_CALL_RESULT` — one Tool message with its own MyMemo-generated `messageId`, the matching `toolCallId`, and bounded client-safe content; recoverable failures use the fixed `Tool failed` content followed by namespaced `CUSTOM` error metadata. A canceled/failed Run may end after an invocation with no result. Legacy uncorrelated Tool rows replay only as namespaced `CUSTOM` frames and never receive fabricated identities
...
**Exposure gate (`src/features/exposure-gate/`):** new agent work is gated by a server-side Statsig gate (`mymemo_agent_split_runtime_enabled`), evaluated on the **trusted identity** (never the body) **after** identity parse and **before** any conversation/run write, on Conversation creation and both Run-admission surfaces. Denied → `403 { error: "Agent is not enabled" }`. It **fails closed**: a Statsig init/eval failure denies new work (a buggy gate cannot fail open). It does **not** replace auth, ownership, DB invariants, or worker fencing, and reconnect/cancellation for existing owned runs must not depend on it. `STATSIG_SERVER_SECRET` backs the production `StatsigExposureGate`; `AGENT_EXPOSURE_BREAK_GLASS=true` swaps in an always-allow gate for local dev / incident response and requires no secret. The secret is never sent to the sandbox or logged.
...
chat-api holds no provider/document/E2B secrets. It admits Runs, consumes retained AG-UI Streams, retains the temporary legacy projector, reads Downloadable-artifact metadata, and signs read-only artifact URLs. The trusted `agent-worker` owns model traffic, scoped document access, E2B sandbox creation, Redis event production, and Downloadable-artifact publication.

### Key Modules

| Path | Purpose |
|------|---------|
| `src/features/conversations/` | Strict request validation and SSE; the route calls the durable store directly for canonical cancellation, while the controller freezes Conversation scope, orchestrates idempotent AG-UI admission, retains legacy queuing, and scopes the temporary legacy interruption path |
...
- `REDIS_URL` — optional authenticated `rediss://` secret for cursorless Live
  preview and retained AG-UI Streams during the staged cutover. Missing,
  malformed, insecure, or unreachable configuration disables only Live
  delivery; boot, health, and durable Postgres state remain operational until
  #344 makes secure Redis mandatory. Never logged or passed into E2B
...
- `REDIS_URL` — optional authenticated `rediss://` secret for cursorless Live
  preview and retained AG-UI Streams during the staged cutover. Missing,
  malformed, insecure, or unreachable configuration disables only Live
  delivery; boot, health, and Run execution remain operational until #344
  makes secure Redis mandatory. Never logged or passed into E2B
- `LIVE_STREAM_ALLOW_INSECURE_LOCAL_REDIS` (default: off) — integration-test-only
