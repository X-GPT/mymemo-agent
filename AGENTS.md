...
| `src/features/conversation-runtime-store/` | Currently unused compatibility re-export over shared `@mymemo/agent-db/runtime-store` records and helpers; retained without expanding its surface. The shared helpers own fenced sandbox/taint mutation plus the unfenced `orphan_sandboxes` recovery ledger, while run-store composes Agent-session pointer publication into terminal transactions |
