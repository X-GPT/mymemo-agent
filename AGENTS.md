...
- **@mymemo/agent-db** (`packages/agent-db/`) - shared writable-DB data layer (`mymemo_agent`): the Drizzle schema, the `Database` client, the concurrency-critical run-store transaction helpers (`startClaimedRunTx`/`appendRunEventTx`/`transitionRunTerminalTx`/`heartbeatRunTx`/`requestRunInterruptionTx`/`markStaleRunsTx`/`admitQueuedRunTx`/`loadRunStartedTx`; `claimNextRunTx` is the retiring Run-scoped claim, now used only by tests), the shared Ownership-epoch predicates for Run state plus the transitional Run-lease predicates for runtime and transcript mutations, the fenced `conversation_runtime`/`orphan_sandboxes` helpers, the SDK session-transcript mirror helpers over the worker-only `agent_sessions` table, the canonical `run_events.type` write-side vocabulary (`RunEventType`), the PGlite test harness, and the migrations. The Claim protocol's concurrency properties — `SKIP LOCKED` contention, the Claim-versus-admission snapshot boundary, a superseded holder racing its successor — are the one thing a single in-process backend cannot express, so they live in `src/conversation-ownership.postgres.test.ts`, gated on `AGENT_DATABASE_URL` and run by the CI `integration` job against the Postgres major production runs. Both runtimes import it so the queue protocol, runtime-persistence fence, and permanent-history event vocabulary share one definition over one `pg` driver. Runtime-store performs sandbox/taint and Agent-session pointer writes; run-store composes its named operations — the pointer publication into qualifying terminal transactions through its transitional user-bound Run lease, and the deliberately unfenced recovery taint into stale-Run recovery; chat-api's `PostgresRunStore` composes transaction-scoped shared admission with the Conversation lifecycle lock
