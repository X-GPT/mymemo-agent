...
- **agentcore-canary-dispatch** (`apps/agentcore-canary-dispatch/`) - production-shaped AgentCore dispatch boundary. It owns the strict content-free SQS envelope and post-commit Acquisition receipt, fail-closed SSM-gated immediate/repair publisher, audited manual replay, partial-batch SQS consumer acknowledgement matrix, AgentCore invocation adapter, and Runtime-side exact-acquisition boundary. It stops after the exact Run reaches `running`; issue #451 attaches shared Run serving
